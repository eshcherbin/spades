...
void PolymorphismsDetection::OutputBubbles(conj_graph_pack& gp, const BubblesContainer& bubbles,
    ...
    INFO("Writing " << bubbles_name << " bubbles to " << cfg::get().output_dir + bubbles_name << ".bubbles");
    std::ofstream simple_out(cfg::get().output_dir + bubbles_name + ".bubbles");
    for (const auto& bubble : bubbles) {
        simple_out << bubble;
    }
    simple_out.flush();
    DEBUG("Collecting " << bubbles_name << " bubbles into a single graph component");
    ...
    DEBUG("Collected " << bubbles_edges.size() << " edges");
